 ...
 async function updateLeaderboardSheet(leaderboardData, telemetryData, lapData) {
  try {
    ...
    for (i = 0; i < leaderboardData.length; i++) { // Loop through latest leaderboard and use reference data to find driver info
      ...
      let cautionStripImg;
      if (flagColor === 'green') {
        flagColorImg = referenceData.leaderboardImages['Green Flag'];
        cautionStripImg = '';
      ...
      } else if (thisCarTelemetryData === undefined){
        thisCarDNF = false;
        thisCarDNFImg = '';
        thisCarSpeed = '';
            ...
            try {
              if (telemetryStartIndex !== -1) {
                ...
                for (i = 0; i < updatedUnofficialLeaderboardData.length; i++) {
                  ...
                  } else if (parseInt(updatedUnofficialLeaderboardData[i].Laps_Behind) > 0) {
